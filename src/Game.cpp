...
void Game::initWindow() {
  ...
  // on
  this->data->create(sf::VideoMode(VIDEOMODE_WIDTH, VIDEOMODE_HEIGHT), "title", sf::Style::Close | sf::Style::Titlebar);
  ...
  while (this->data->pollEvent(this->ev)) {
    switch (this->ev.type) {
      ...
      case sf::Event::KeyPressed:
        if (ev.key.code == sf::Keyboard::Escape) this->data->close();
...
void Game::update() {  // update game variables before rendered
  this->pollEvents();
  this->player->Gravity();
  if (this->collision.checkPlayerPlatformCollison(this->player->GetPlayer(), this->platformObj->getPlatform())) {
    std::cout << "collided" << this->platformObj->getPlatform().getPosition().y << std::endl;
