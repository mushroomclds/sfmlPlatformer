...
Game::~Game() {   // deconstrutor definition
  // delete this->data;
  std::cout << "shared ptr count at Game destructor: " << this->data.use_count()
            << std::endl;
}
/*=============================================================================
...
void Game::initWindow() {
  ...
  // on
  this->data->create(sf::VideoMode(VIDEOMODE_WIDTH, VIDEOMODE_HEIGHT), "title",
...
void Game::update() {   // update game variables before rendered
  ...
  if (this->collision.checkPlayerPlatformCollison(
    ...
    std::cout << "collided" << this->platformObj->getPlatform().getPosition().y
              << std::endl;
